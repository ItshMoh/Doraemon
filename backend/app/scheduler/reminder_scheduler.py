...
# Reminder times in IST (Asia/Kolkata): 9AM, 12PM, 3PM, 6PM, 9PM
...
class ReminderScheduler:
    ...
    def start(self) -> None:
        ...
        for hour in REMINDER_HOURS:
            self.scheduler.add_job(
                ...
                trigger=CronTrigger(hour=hour, minute=0, timezone=self.tz),
                id=f"reminder_{hour:02d}",
                replace_existing=True,
            )
            logger.info("Scheduled reminder job at %02d:00 %s", hour, settings.app_timezone)

        self.scheduler.start()
        logger.info("Reminder scheduler started with %d jobs", len(REMINDER_HOURS))
    ...
    async def _send_reminders(self) -> None:
        """Core logic: find open tasks due today, send Telegram reminders."""
        now = datetime.now(self.tz)
        today_str = now.strftime("%Y-%m-%d")
        # scheduled_for marks the exact time slot (top of the hour)
        scheduled_for = now.replace(minute=0, second=0, microsecond=0).isoformat()

        logger.info("Running reminder cycle for %s at %s", today_str, scheduled_for)

        tasks = self.repo.get_open_tasks_due(today_str)
